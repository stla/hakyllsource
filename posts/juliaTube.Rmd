...
[this blog post](http://wordpress.discretization.de/ddg2016/2016/05/10/tutorial-3-framed-closed-curves/), 
in Julia and R. In fact it is useless with R, because the **rgl** function 
`cylinder3d` is faster and better.
...
The current version of **MeshViz** always adds normals to the meshes, so we 
can't correctly see what happens if we only set a couple of sides and if we use 
some twists. So let's see what this gives with the R version, with four sides 
and two twists:
