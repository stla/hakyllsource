...
Denoting by $F(z \,|\, m)$ the incomplete elliptic function of first kind, the 
function
$$
\varphi(z) = -\sqrt{i} \, F\bigl(i \sinh^{-1}(\sqrt{i} \, z) \,|\, -1 \bigr).
$$
is a conformal mapping from the square $[-1,1] \times [-1,1]$ to a centered 
circle. I learnt that [here](https://math.stackexchange.com/a/3159827/38217).

Then, if one has a square image, we can transform it to a circular image with 
the help of this function $\varphi$. I will give an example.
...
First, let's plot a beautiful complex function on a square. I take 
...
stored in the `ZETA` matrix. To do so, we introduce two functions, one to 
interpolate the real parts, the other one to interpolate the imaginary 
parts:
...
Now it's easy. With the help of these two functions, we map the values of 
$\psi$ that we stored in the `PSI` matrix:
