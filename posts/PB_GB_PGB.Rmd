...
$\mathcal{G}\mathcal{B}$ distribution, because the above equality 
makes sense only for $\beta > \alpha$, whereas the constraints on 
the parameters of $CH_1(\nu, \alpha, \beta, \theta)$ are $\nu>0$, 
$\alpha>\nu$, $\boxed{\beta > \nu}$ and $\theta > 0$. 
Thus the constraints on the parameters in 
$CH_1(\nu, a+\nu, a+b+\nu, 1/\theta)$ are $\nu>0$, $a>0$, 
$\boxed{b>-a}$, $\theta > 0$.
