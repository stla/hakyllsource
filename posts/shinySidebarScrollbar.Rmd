...
It is easy to have a scrollbar for the sidebar of a Shiny app: one just has 
to use the CSS `height: 90vh; overflow-y: auto;`:
...
But, as you can see, there is an issue with the `shinyWidgets::dropdown`: when 
one opens it, the part outside the sidebar is hidden. 
Here is a JavaScript solution for this issue:
...
I find the native HTML scrollbars a bit ugly. It is possible to customize them: 
[Styling scrollbars in CSS](https://css-tricks.com/the-current-state-of-styling-scrollbars-in-css/).