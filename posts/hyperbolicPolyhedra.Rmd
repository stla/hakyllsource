...
appropriate when there are duplicated vertices in the mesh (it adds different 
normals to the duplicated vertices). 
And there are (a lot of) duplicated vertices in the mesh created by the 
`gyrosubdiv` function. 

To remove the duplicated vertices, one can use the `vcgClean` function of the 
`Rvcg` package (which also adds the normals). Observe the difference:

```{r triangle-dupVertices, rgl=TRUE}
idx <- faces[1,]
A <- vertices[idx[1],]
B <- vertices[idx[2],]
C <- vertices[idx[3],]
...
```{r triangle-undupVertices, rgl=TRUE}
